%%  function to calculate 3rd central moment for BL original model with random eta
%   i.e. MBL

function thirdmoment = blr_cmom3e(theta,h)

eps=0;
lambda = theta(:,1);
mux = theta(:,4); 
sigmax = mux;
%     alpha = theta(5); nu =theta(5)./theta(6); kappa=theta(7); phi=theta(8); eps = theta(9)
alpha = theta(:,5); nu =theta(:,6); kappa=theta(:,2); phi=theta(:,3);
...
%    thirdmoment=thirdmoment-3*MBL_avg(theta,h).*MBL_var(theta,h)-MBL_avg(theta,h);
