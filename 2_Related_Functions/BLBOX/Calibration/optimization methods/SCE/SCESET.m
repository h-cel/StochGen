...
%
%   OPTIONS = SCESET(OLDOPTS,'PARAM1',VALUE1,...) creates a copy of OLDOPTS
%   with the named parameters altered with the specified values.
%
%   OPTIONS = SCESET(OLDOPTS,NEWOPTS) combines an existing options structure
...
%   OLDOPTS.
%
%   SCESET with no input arguments and no output arguments displays all
...
%   SCESET(OPTIMFUNCTION) to see options for a specific function.
%
%   OPTIONS = SCESET (with no input arguments) creates an options structure
...
if (nargin == 0) && (nargout == 0)
    fprintf('                   nCOMPLEXES: [ positive scalar {5} ]\n');
    fprintf('             nITER_INNER_LOOP: [ positive scalar {20} ]\n');
