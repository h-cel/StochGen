function [X,FVAL,EXITFLAG,OUTPUT] = parSCE(FUN,X0,LB,UB,OPTIONS,varargin)
...
if my_rank==0
    ...
    for i = 2:OPTIONS.MAX_ITER,
        ...
        for j = 1:OPTIONS.nCOMPLEXES,
            ...
            %submit new jobs
                ...
                MPI_Send(j,tagcomplex+j,comm,COMPLEX,COMPLEX_FITNESS,k1,k2);

        end
        %gather new results
        [message_rank message_tag]=MPI_Probe('*',tagcomplexback,comm);
        while ~isempty(message_rank)
             for message_counter=1:length(message_rank)
                [COMPLEX,COMPLEX_FITNESS,k1,k2]=MPI_Recv(message_rank(message_counter),tagcomplexback,comm);           
    ...
    while ~timetoquit
        ...
        if isempty(find(message_rank==0,1)) %if no messages from master
            for message_counter=1:length(message_tag) 
                MPI_Recv(message_rank(message_counter),message_tag(message_counter),comm); % then clean other messages
            end
        else
            for message_counter=find(message_rank==0) %else, loop through messages from master
                if message_tag(message_counter)==tagcomplex+my_rank
                    [COMPLEX,COMPLEX_FITNESS,k1,k2]=MPI_Recv(0,tagcomplex+my_rank,comm);
                    ...
                    MPI_Send(0,tagcomplexback,comm,COMPLEX,COMPLEX_FITNESS,k1,k2);
                      
                elseif message_tag(message_counter)==tagquit %request to quit
                    MPI_Recv(0,tagquit,comm);
                    timetoquit=1;
                    break;
                end
            end
