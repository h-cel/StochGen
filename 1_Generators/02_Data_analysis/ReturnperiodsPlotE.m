...
time = matload(timename);
yrsdata = unique(time(:,1));
if iscell(orig) == 1
...
for i=1:repetitions
    for j = 1:repetitions
        for k=1:repetitions
            [T_data, Ex_data] = returnperiods(data{i,j}(:,k+size(data{1,1},2)-repetitions), yrsdata, agg);
...
if iscell(orig) == 1
    [T_orig, Ex_orig] = returnperiods(orig{1}(:,4), yrsorig, agg);
else
    [T_orig, Ex_orig] = returnperiods(orig(:,4), yrsorig, agg);
end
