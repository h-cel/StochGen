...
for n=1:nrepetitions
    ...
    %expanding edges
    if edgesho(1)>min(tsmod(:,n))
        edgesho(1) = min(tsmod(:,n));
    end
    if edgesho(end)<max(tsmod(:,n))
        edgesho(end) = max(tsmod(:,n));
    end
    
    [Nfa] = histcounts(tsmod(:,n), edgesho, 'Normalization', 'probability');
