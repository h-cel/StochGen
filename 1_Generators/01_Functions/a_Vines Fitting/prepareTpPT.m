function prepareTpPT(data,modelname)
%% prepareTpPT This function builds the monthly C-vines for the configuration TpPT
%   Set the data in the correct form to build the monthly C-vines
%   Monthly datasets will be built as follows: 3-tuples (T(j), P(j+1), T(j+1))
...
for i = 1:12
    dataMonth = data(data(:,2)==i,4:5); %Selection of data for the current month (out of all years)
    ...
    for k =  1:length(Xst)
        ...
        %other
        X = [X; dataMonth(Xst(k):Xend(k),1)];
        Y = [Y; dataMonth(Yst(k):Yend(k),2)];
        Z = [Z; dataMonth(Yst(k):Yend(k),1)];
    end
    filename = sprintf('TpPT_%d.mat',i); %Different filename for each month
    D = [X Y Z];
